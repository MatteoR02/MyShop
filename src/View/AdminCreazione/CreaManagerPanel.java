...
public class CreaManagerPanel extends JPanel {
    ...
    public CreaManagerPanel(MainPage frame) {
        ...
        AdminListener adminListener = AdminListenerBuilder.newBuilder(frame).comboBoxPV(selPuntoVenditaBox).
                fieldNome(nome).fieldCognome(cognome).fieldEmail(email).fieldTelefono(telefono).fieldUsername(username).
                fieldPassword(password).dateChooserNascita(dataNascita).fieldNazione(nazione).fieldCitta(citta).fieldCap(cap).
                fieldVia(via).fieldCivico(civico).build();
