...
public class CreaServizioPanel extends JPanel {
    ...
    public CreaServizioPanel(JFrame frame, PuntoVendita pv, boolean modifica, ComponenteCatalogo comp) {
        ...
        AdminListener adminListener = AdminListenerBuilder.newBuilder((MainPage) frame).fieldNome(fieldNome)
                .fieldDescrizione(fieldDescrizione).fieldPrezzo(fieldPrezzo).
                comboBoxCategoria(selCategoriaBox).comboBoxErogatore(selErogatoriBox).
                labelImmagini(labelAllegato).puntoVendita(pv).build();
