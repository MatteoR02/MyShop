...
public class CreaProdottoPanel extends JPanel {
    ...
    public CreaProdottoPanel(JFrame frame, PuntoVendita pv, boolean modifica, ComponenteCatalogo comp) {
        ...
        AdminListener adminListener = AdminListenerBuilder.newBuilder((MainPage) frame). fieldNome(fieldNome).
                fieldDescrizione(fieldDescrizione).fieldPrezzo(fieldPrezzo).fieldCorisa(fieldCorsia).
                fieldScaffale(fieldScaffale).comboBoxMagazzino(selMagazzinoBox).comboBoxCategoria(selCategoriaBox).
                comboBoxErogatore(selErogatoriBox).labelImmagini(labelAllegato).puntoVendita(pv).build();
