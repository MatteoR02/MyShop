...
public class AddRecensionePanel extends JPanel {
    ...
    public AddRecensionePanel(JFrame frame, JDialog dialog, ComponenteCatalogo comp, boolean fromManager, Recensione recensione){
        ...
        CatalogoListener catalogoListener = CatalogoListenerBuilder.newBuilder((MainPage) frame).dialog(dialog).
                fieldTitolo(fieldTitolo).fieldTesto(fieldTesto).sliderValutazione(sliderValutazione).compCatalogo(comp).build();
        ...
        if (fromManager){
            ManagerListener managerListener = ManagerListenerBuilder.newBuilder((MainPage) frame).dialog(dialog).fieldTitolo(fieldTitolo).
                    fieldTesto(fieldTesto).compCatalogo(comp).recensione(recensione).build();
