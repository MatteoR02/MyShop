...
import View.ViewModel.RecensioneModel;
...
public class InfoArticoloPanel extends JPanel {
    ...
    public InfoArticoloPanel(MainPage frame, ComponenteCatalogo comp, boolean ordinato, ArrayList<Recensione> listaRecensioni){
        ...
        CatalogoListener catalogoListenerFoto = CatalogoListenerBuilder.newBuilder(frame).compCatalogo(comp).
                labelImmagine(immagine).labelIndex(labelIndexFoto).build();
        ...
        if (!ordinato){
            ...
            for (Recensione rec: recensioni) {
                RecensioneModel recensioneModel = new RecensioneModel(rec, comp, frame, false);
                ...
                if (resultRisposta.getResult()== ExecuteResult.ResultStatement.OK){
                    ...
                    RecensioneModel rispostaList = new RecensioneModel(risposta, comp, frame, true);
                    recensioniScrollPanel.add(recensioneModel);
        ...
        CatalogoListener catalogoListenerSort = CatalogoListenerBuilder.newBuilder(frame).arrayRecensioni(resultRec.getObject()).
                comboBoxOrdinamento(ordinamento).compCatalogo(comp).build();
